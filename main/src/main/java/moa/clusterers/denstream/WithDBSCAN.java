...
public class WithDBSCAN extends AbstractClusterer {
	...
	public IntOption initPointsOption = new IntOption("initPoints", 'i',
	 ...
	 "offline multiplier for epsilion.", 1, 1, 20);
...
//		lambda = lambdaOption.getValue();
		...
		processingSpeed = speedOption.getValue();
                ...
                initPointsOption.setValue(500);
                epsilon = 0.3;
                beta = 0.3;
                lambda = 0.5;
                minPoints = 2;
                offlineOption.setValue(2);
	...
	public void trainOnInstanceImpl(Instance inst) {
		...
		if (numProcessedPerUnit % (processingSpeed+1) == 0) {
			timestamp++;
		...
		if (!initialized) {
			...
			if (!merged) {
                                double[] arr = point.toDoubleArray();
				o_micro_cluster.getClustering().add(
						new MicroCluster(arr, arr.length, timestamp, lambda,
								currentTimestamp));
			}
	...
	private MicroCluster nearestCluster(DenPoint p, Clustering cl) {
        ...
        public Clustering getClusteringResult() {
            ...
            Clustering res = dbscan.getClustering(p_micro_cluster);
            return res;
        }
        
	public Clustering getClusteringResult(Clustering prevClustering) {
            DBScan dbscan = new DBScan(p_micro_cluster,offlineOption.getValue() * epsilon, minPoints);
            Clustering res = dbscan.getClustering(p_micro_cluster);
            setIdsOfClusters(res,prevClustering); 
            return res;
	}
        
         private void setIdsOfClusters(Clustering currentC, Clustering previousC) {
            if(previousC == null || currentC == null){
                return;
            }
            // FIND BEST COUPLE 
            ...
            while(couples.size() < curClusters.size()  && couples.size() < prevClusters.size()){
                Integer best1 = null;
                Integer best2 = null;
                double mindist = Double.MAX_VALUE;
                for(int curid = 0; curid < curClusters.size(); curid++){
                    if(couples.containsKey(curid)){
                       continue; 
                    }
                    for(int previd = 0; previd < prevClusters.size(); previd++){
                ...
                couples.put(best1, best2);
            }
            for(Entry<Integer,Integer> couple : couples.entrySet()){
                currentC.getClustering().get(couple.getKey()).setId(couple.getValue());
            }
