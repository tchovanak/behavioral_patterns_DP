...
public class UserModelPPSDM {
    ...
    private BlockingQueue<List<Integer>> lastSessions; // queue of last n user sessions
    ...
    */
    ...
    /*
        Updates user model with new instance representing one session
    */
    public void updateWithInstance(Instance inst, Map<Integer, Integer> catMap) {
        List<Integer> session = new LinkedList<>();
        for(int i = 2; i < inst.numValues(); i++){
            session.add(catMap.get((int)Math.round(inst.value(i))));
        }
        if(!lastSessions.offer(session)){
            lastSessions.poll();
            lastSessions.offer(session);
