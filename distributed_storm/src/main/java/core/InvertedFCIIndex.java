...
public class InvertedFCIIndex implements Serializable {
    ...
    protected class IDArrayList implements Serializable{
        ...
        public void removeID(int fciSize, Integer id) {
            if (this.map.containsKey(fciSize)) {
                this.map.get(fciSize).remove(id);
                if (this.map.get(fciSize).isEmpty()) {
                    this.map.remove(fciSize);
                    this.sortCheck.remove(fciSize);
                }
            }
        }
    ...
    public void removeSemiFCI(SemiFCI itemset) {
        for (Integer item : itemset.getItems()) {
            if (!this.invertedIndex.containsKey(item)) continue;
            IDArrayList list = this.invertedIndex.get(item);
            list.removeID(itemset.getId().getDimension(), itemset.getId().getPosition());
            if (!list.map.isEmpty()) continue;
            this.invertedIndex.remove(item);
        }
    }
